...
<Warning>
**Email Rate Limit:** OptiDev Cloud uses Supabase's built-in email service, which has a limit of **2 emails per hour**. This applies to invitations, magic links, and password reset emails. Custom SMTP provider support is planned for a future release
</Warning>
...
The email rate limit applies to invitations, magic links, and password resets. For production apps with higher email volume, configure a custom SMTP provider (SendGrid, Resend, etc.) in your Supabase project settings.
