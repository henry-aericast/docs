...
The details view includes three tabs for monitoring your function. Use the time filter dropdown to view data from the last 15 minutes, 1 hour, 3 hours, or 1 day.

**Overview** - Key metrics at a glance:
- Total invocations and logs count
- Average execution time (ms)
- Success and error counts
- Success rate percentage

Monitor success rates and execution times to spot performance issues.

**Invocations** - HTTP request history:
- Timestamp, method, status code, and path for each request
- Click any row to see full request/response details
- Color-coded status (green for success, red for errors)

Check status codes to identify failed requests and debug API issues.

**Console Logs** - Function output:
- All `console.log()`, `console.error()`, etc. from your function
- Log level indicators (log, info, warn, error, debug)
- Expandable rows for full message details

Use `console.log()` in your function code to add debug information.

<Note>
Metrics and logs are retained for a limited time. Check regularly if you need to debug issues.
</Note>
...
Store your API keys securely in the **Secrets** tab, then ask OptiDev Agent to use them:

> *"Create a Stripe payment function using my STRIPE_SECRET_KEY secret"*

OptiDev Agent will write code that securely accesses your secret without exposing it in your frontend.
